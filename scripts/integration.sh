...
curl -sL $ADDRESS_DC2/v1/kv/backup/$EXCLUDED_KEY/nodelete | grep -q "ZG9uJ3QgZGVsZXRl" || (curl -sL $ADDRESS_DC2/v1/kv/backup/$EXCLUDED_KEY/nodelete && exit 1)
...
echo "    Checking key still exists in DC2"
curl -sL $ADDRESS_DC2/v1/kv/backup/parent_folder/test_folder/testkey | grep -q "ZG9uJ3QgZGVsZXRl" || (curl -sL $ADDRESS_DC2/v1/kv/backup/parent_folder/test_folder/testkey && exit 1)
